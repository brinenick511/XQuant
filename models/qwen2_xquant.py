...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None):
        ...
        k_q = [int(l[0]),config.num_hidden_layers]
        v_q = [int(l[1]),config.num_hidden_layers]
        ...
        if self.layer_idx >= v_m and self.layer_idx % 2 == 1:
            self.vm=True
        print(self.layer_idx,self.k_bits,self.v_bits,self.km,self.vm)
        # ADDED >>
...
class Qwen2FlashAttention2_XQuant(Qwen2Attention):
    """
    ...
    def forward(
        ...
        kv_seq_len = key_states.shape[-2]
        if past_key_value is not None and len(past_key_value)==self.config.num_hidden_layers:
            if self.layer_idx is None:
                raise ValueError(
                    f"The cache structure has changed since version v4.36. If you are using {self.__class__.__name__} "
                    "for auto-regressive decoding with k/v caching, please make sure to initialize the attention class "
        ...
        if past_key_value is not None:
            ...
            cache_has_contents = len(past_key_value)==self.config.num_hidden_layers and past_key_value[self.layer_idx][-1] > 0
            
            # WHICH MEANS (seq_len > 32k) not possible
        ...
        if past_key_value is None or len(past_key_value)<self.config.num_hidden_layers: # PREFILL
            # print(f'prefill #{self.layer_idx}')
            # repeat k/v heads if n_sheads < n_heads
            ...
            attn_output = _flash_attention_forward(
                ...
                key_states_rep.transpose(1, 2),
                value_states_rep.transpose(1, 2),
                attention_mask,
                q_len,
            ...
            if key_states_quant is not None:
                ...
                if self.km == True:
                    ...
                    if self.kq==2:
                        anno=2
                key_states_quant_trans, key_scale_trans, key_mn_trans = triton_quantize_and_pack_along_last_dim(key_states_quant.transpose(2, 3).contiguous(), self.group_size, self.k_bits,anno)
                # TODO: Cali_K
                if self.kq == 1:
        ...
        elif past_key_value is not None and len(past_key_value) == self.config.num_hidden_layers: # DECODE
            ...
            if key_states_quant_trans is not None:
                if self.k_bits == 1 or True:
                    # key_states_quant_trans=repeat_kv(key_states_quant_trans, self.num_key_value_groups)
                    # key_scale_trans=repeat_kv(key_scale_trans, self.num_key_value_groups)
                    # key_mn_trans=repeat_kv(key_mn_trans, self.num_key_value_groups)
                    # print(self.group_size,query_states.shape,key_states_quant_trans.shape,key_scale_trans.shape,key_mn_trans.shape,self.k_bits)
                    # att_qkquant = triton_bmm_fA_qB_outer(self.group_size, query_states, key_states_quant_trans, key_scale_trans, key_mn_trans, self.k_bits)
                    att_qkquant = triton_bmm_fA_qB_outer(self.group_size, query_states, repeat_kv(key_states_quant_trans, self.num_key_value_groups), repeat_kv(key_scale_trans, self.num_key_value_groups), repeat_kv(key_mn_trans, self.num_key_value_groups), self.k_bits)
            ...
            if key_states_full is not None:
                # print(key_states_full.device,key_states.device)
                # print(key_states_full.dtype,key_states.dtype)
                # key_states_full = key_states_full.to(key_states.device).detach().clone()
            ...
            if value_states_quant is None:
                ...
                if self.v_bits == 1 or True:
                    # value_states_quant=repeat_kv(value_states_quant, self.num_key_value_groups)
                    # value_scale=repeat_kv(value_scale, self.num_key_value_groups)
                    # value_mn=repeat_kv(value_mn, self.num_key_value_groups)
                    # attn_output = triton_bmm_fA_qB_outer(
                        # self.group_size, attn_weights[:, :, :, :-value_full_length],
                        # value_states_quant, value_scale, value_mn, self.v_bits)
                    attn_output = triton_bmm_fA_qB_outer(
                        self.group_size, attn_weights[:, :, :, :-value_full_length],
                        repeat_kv(value_states_quant, self.num_key_value_groups), repeat_kv(value_scale, self.num_key_value_groups), repeat_kv(value_mn, self.num_key_value_groups), self.v_bits)
                else:
            ...
            if value_full_length > self.residual_length:
                # if self.layer_idx==21: print('new_value')
        ...
        if key_states_full is not None:
            key_states_full = key_states_full.detach().clone()
        if value_states_full is not None:
            value_states_full = value_states_full.detach().clone()
        ...
        elif len(past_key_value)<self.config.num_hidden_layers:
            past_key_value.append((key_states_quant_trans, key_states_full, key_scale_trans, key_mn_trans, 
                        value_states_quant, value_states_full, value_scale, value_mn, kv_seq_len))
        else:
            past_key_value[self.layer_idx] = (key_states_quant_trans, key_states_full, key_scale_trans, key_mn_trans, 
                        value_states_quant, value_states_full, value_scale, value_mn, kv_seq_len)
        # attn_output = attn_output.reshape(bsz, q_len, self.hidden_size)
...
QWEN2_ATTENTION_CLASSES = {
    ...
    "flash_attention_2": Qwen2FlashAttention2_XQuant,
    "sdpa": Qwen2SdpaAttention,
    # "xquant": Qwen2FlashAttention2_XQuant,
}
...
class Qwen2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # if self.layer_idx ==23 and len(past_key_value)==24:
        #     suf=['quantized','residual','scaling-factor','zero-point']
        #     ty=['key','value']
        #     for ii in range(8):
        #         print(f'[ {ty[ii//4]} - {suf[ii%4]} ]')
        #         if past_key_value[23][ii] is not None:
        #             print(past_key_value[23][ii].shape,past_key_value[23][ii].dtype)
        #         else:
        #             print('NONE')

        # Self Attention
        with torch.cuda.device(f'cuda:{self.hf_device}'):
    ...
    # if self.layer_idx ==21:
        ...
        # for ii in range(8):
        #     print(f'[ {ty[ii//4]} - {suf[ii%4]} ]')
        #     if present_key_value[self.layer_idx][ii] is not None:
        #         print(present_key_value[self.layer_idx][ii].shape,present_key_value[self.layer_idx][ii].dtype,present_key_value[self.layer_idx][ii].device)
        #     else:
        #         print('NONE')
...
class Qwen2Model_XQuant(Qwen2PreTrainedModel):
    """
    ...
    def forward(
        ...
        # elif self.cnt>0:
        #     for ii in range(8):
        #         print(past_key_values[ii].dtype,past_key_values[ii].shape,)
    ...
    def _update_causal_mask(
        ...
        # past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0
        past_seen_tokens = past_key_values[0][-1] if past_key_values is not None else 0
        using_static_cache = isinstance(past_key_values, StaticCache) # FALSE
...
class Qwen2ForCausalLM_XQuant(Qwen2PreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = Qwen2Model_XQuant(config)
        self.vocab_size = config.vocab_size
