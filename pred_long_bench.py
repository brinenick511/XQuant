...
from transformers import AutoConfig, LlamaConfig, MistralConfig, AutoTokenizer
...
def build_chat(tokenizer, prompt, model_name):
    ...
    if "mistral-v0.2-instruct" in model_name.lower():
        messages = [
...
def get_pred(model, tokenizer, data, max_length, max_gen, prompt_format, dataset, device, model_name, out_path):
    ...
    for json_obj in tqdm(data):
        ...
        # output = output[context_length:].to(tokenizer.device)
        output = output[context_length:].to(device)
...
if __name__ == '__main__':
    ...
    if 'qwen' in model_args.model_name_or_path.lower():
        if model_args.k_bits < 16 and model_args.v_bits < 16:
            from models.qwen2_xquant import Qwen2ForCausalLM_XQuant
            ...
            if is_multi_gpu:
                with init_empty_weights():
                    model = Qwen2ForCausalLM_XQuant.from_pretrained(
                ...
                for i in range(config.num_hidden_layers):
                    # print(model.model.layers[i].hf_device)
                    model.model.layers[i].hf_device = model.hf_device_map[f'model.layers.{i}']
